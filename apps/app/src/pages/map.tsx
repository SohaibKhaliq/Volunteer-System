  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)]">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* List View */}
        <div className={`w-full md:w-1/2 lg:w-2/5 overflow-y-auto p-4 space-y-4 ${viewMode === 'map' ? 'hidden md:block' : 'block'}`}>
          <div className="flex justify-between items-center mb-2">
            <h2 className="font-semibold text-lg">{t('Available Opportunities')}</h2>
            <span className="text-sm text-muted-foreground">{MOCK_EVENTS.length} {t('results')}</span>
          </div>
            ...
            <Card key={event.id} className="hover:shadow-md transition-shadow">
              <div className="flex flex-col sm:flex-row h-full">
                <div className="w-full sm:w-32 h-32 sm:h-auto bg-slate-100 relative shrink-0">
                  <img src={event.image} alt={event.title} className="absolute inset-0 w-full h-full object-cover rounded-t-lg sm:rounded-l-lg sm:rounded-tr-none" />
                </div>
                <div className="flex-1 p-4 flex flex-col justify-between">
                  ...
                  <div className="mt-4 flex justify-end">
                    <Link to={`/detail/event/${event.id}`}>
                      <Button size="sm" variant="outline">{t('View Details')}</Button>
                    </Link>
                  </div>
                </div>
              </div>
              ...
              <Marker key={event.id} position={event.coordinates as [number, number]}>
                <Popup>
                  <div className="p-2 min-w-[200px]">
                    ...
                    <Link to={`/detail/event/${event.id}`}>
                      <Button size="sm" className="w-full h-8 text-xs">{t('View')}</Button>
                    </Link>
                  </div>
                </Popup>
              </Marker>
