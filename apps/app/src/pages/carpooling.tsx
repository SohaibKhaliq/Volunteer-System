      ...
      <div className="relative overflow-hidden bg-slate-900 pt-24 pb-32">
        ...
        <div className="container relative px-4 mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-white mb-6">
            ...
            <span className="text-slate-400">{t('Save More')}</span>
          </h1>
          ...
          <Card className="border-border shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50 rounded-xl bg-card overflow-hidden">
            <CardContent className="p-6 md:p-8">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant={'outline'}
                      className={cn(
                        'w-full h-14 justify-start text-left font-bold rounded-lg border-border bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800',
                        !date && 'text-muted-foreground'
                      )}
                    >
                      <CalendarIcon className="mr-3 h-5 w-5 text-primary opacity-50" />
                      {date ? safeFormatDate(date, 'PPP') : <span>{t('Pick a date')}</span>}
                    </Button>
                  </PopoverTrigger>
                ...
                <Button className="w-full h-14 rounded-lg text-lg font-bold shadow-xl shadow-primary/20 hover:translate-y-[-1px] transition-all duration-300">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              <Button
                ...
                className={cn(
                  'rounded-lg px-6 font-bold transition-all',
                  filtersOpen
              ...
              <div className="flex gap-2">
                <Badge
                  variant="secondary"
                  className="rounded-md px-3 py-1 font-black uppercase tracking-widest text-[10px] bg-slate-100 dark:bg-slate-800 border-none"
                >
            ...
            <div className="flex gap-3 w-full md:w-auto">
              <Button
                variant="outline"
                onClick={() => navigate('/transport-request')}
                className="flex-1 md:flex-none rounded-lg font-bold border-primary/20 hover:bg-primary/5 transition-all"
              >
                {t('Request a Ride')}
              </Button>
              <Button
                onClick={() => navigate('/transport-offer')}
                className="flex-1 md:flex-none rounded-lg font-bold shadow-xl shadow-primary/10 transition-all"
              >
                {t('Offer a Ride')}
              </Button>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div
              className={cn(
                ...
                filtersOpen
                  ...
                  : 'opacity-0 -translate-y-4 pointer-events-none lg:block lg:opacity-100 lg:translate-y-0 lg:pointer-events-auto'
              )}
            >
              <Card className="border-border shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50 rounded-xl bg-card overflow-hidden">
                <CardHeader className="bg-slate-50/50 dark:bg-slate-950/50 pb-4 border-b border-border">
                  <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                    <Filter className="h-3.5 w-3.5" /> {t('Refine Search')}
                  </CardTitle>
                </CardHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3">
                      {['All', 'Offers', 'Requests'].map((label) => (
                        <label
                          key={label}
                          className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors cursor-pointer border border-transparent hover:border-border/50 group"
                        >
                          <input
                            type="checkbox"
            ...
            <div className="lg:col-span-3 space-y-6">
              {(rides || []).map((ride: any) => (
                <Card
                  key={ride.id}
                  className="border-border hover:border-primary/50 shadow-lg hover:shadow-2xl hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50 rounded-xl bg-card overflow-hidden transition-all duration-300 group cursor-pointer"
                >
                  <CardContent className="p-0">
                    <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-border/50">
                      ...
                      <div className="flex-1 p-8 space-y-8">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-3">
                            <Badge
                              className={cn(
                                'px-2 py-0.5 rounded-md font-black uppercase tracking-widest text-[10px] border-none',
                                ride.type === 'Offer' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                              )}
                              variant="outline"
                            >
                              {t(ride.type)}
                        ...
                        <div className="relative pl-10 border-l-2 border-dashed border-border/50 space-y-8 py-2 ml-4">
                          <div className="relative">
                            <div className="absolute -left-[37px] top-1.5 h-6 w-6 rounded-full border-4 border-background bg-primary shadow-lg" />
                            <div className="space-y-1">
                              <p className="font-bold text-xl tracking-tight leading-tight">{ride.time}</p>
                              <p className="text-muted-foreground font-medium text-base">{ride.from}</p>
                            </div>
                          </div>
                          <div className="relative">
                      ...
                      {/* Side Info */}
                      <div className="md:w-64 bg-slate-50/50 dark:bg-slate-950/20 p-8 flex flex-col justify-between items-center text-center gap-6 border-l border-border">
                        <div className="space-y-4">
                          <div className="relative inline-block">
                            <Avatar className="h-16 w-16 ring-4 ring-background shadow-2xl border-none">
                              <AvatarImage src={ride.driver?.image} className="object-cover" />
                              <AvatarFallback className="bg-slate-100 dark:bg-slate-800 text-slate-400 text-xl font-bold">
                                {ride.driver?.name?.[0] || '?'}
                              </AvatarFallback>
                            </Avatar>
                            <div className="absolute -bottom-1 -right-1 bg-white dark:bg-slate-900 rounded-lg px-2 py-0.5 text-[10px] font-black shadow-md border border-border">
                              ★ {ride.driver?.rating || 'N/A'}
                            </div>
                          </div>
                          <div>
                            <p className="font-bold text-lg text-slate-900 dark:text-slate-100 tracking-tight">
                              {ride.driver?.name || t('Unknown Driver')}
                            </p>
                        ...
                        <Button className="w-full rounded-lg h-12 font-bold tracking-tight shadow-xl shadow-primary/10 hover:shadow-2xl hover:translate-y-[-1px] transition-all">
                          {t('Reserve Seat')}
                        </Button>
                      </div>
                    </div>
