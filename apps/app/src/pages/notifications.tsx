...
export default function NotificationsPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="relative overflow-hidden bg-slate-900 pt-24 pb-32 mb-8">
      ...
      <div className="container px-4 mx-auto">
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 sticky top-4 z-20 bg-background/80 backdrop-blur-md p-4 rounded-xl border border-border shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-primary/10 rounded-lg text-primary font-black text-[10px] uppercase tracking-widest px-3">
              ...
              <Button
                ...
                className="rounded-lg font-bold hover:bg-slate-100 dark:hover:bg-slate-800"
              >
                {t('Clear Selection')}
              </Button>
            </div>
            <div className="flex items-center gap-2 w-full md:w-auto">
              <Button
                ...
                disabled={selected.length === 0}
                className="flex-1 md:flex-none rounded-lg font-bold border-primary/20 hover:bg-primary/5 h-10 px-4"
              >
                {t('Mark Read')}
              </Button>
              <Button
                ...
                disabled={selected.length === 0}
                className="flex-1 md:flex-none rounded-lg font-bold border-border h-10 px-4"
              >
                {t('Mark Unread')}
              </Button>
            </div>
          ...
          <div className="space-y-4">
            {isLoading && itemsArr.length === 0 ? (
              [1, 2, 3].map((i) => <SkeletonCard key={i} className="rounded-xl border-border shadow-sm" />)
            ) : itemsArr.length === 0 ? (
              <div className="py-24 text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold tracking-tight">{t('No notifications yet')}</h3>
              ...
              itemsArr.map((n: any) => {
                ...
                return (
                  <Card
                    key={n.id}
                    className={cn(
                      "border-border rounded-xl transition-all duration-300 group overflow-hidden bg-card",
                      !n.read ? "shadow-2xl shadow-primary/5 bg-slate-50/50 dark:bg-slate-900/50 border-primary/20" : "opacity-80 hover:opacity-100 border-transparent hover:border-border hover:shadow-lg",
                  ...
                  >
                    <CardContent className="p-0">
                      <div className="flex flex-col md:flex-row items-stretch">
                        ...
                        <div className="flex-1 p-6 md:p-8 md:pl-0 space-y-4">
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <Badge variant="secondary" className="rounded-md bg-slate-100 dark:bg-slate-800 font-black uppercase tracking-widest text-[10px] px-2 py-0.5 border-none">
                            ...
                            <time className="text-[10px] font-black uppercase tracking-widest text-muted-foreground whitespace-nowrap bg-slate-50 dark:bg-slate-900 px-3 py-1.5 rounded-md border border-border">
                              {safeFormatDate(createdAt)} • {safeFormatTime(createdAt, 'hh:mm a')}
                            </time>
                          </div>

                          <div className="flex items-start gap-4">
                            {payload.image && (
                              <div className="relative shrink-0 overflow-hidden rounded-xl border border-border h-20 w-20 shadow-xl group-hover:scale-105 transition-transform">
                                <img src={payload.image} className="h-full w-full object-cover" alt="" />
                              </div>
                            )}
                            <div className="space-y-1 flex-1">
                              <h4 className="text-lg font-bold tracking-tight text-foreground leading-snug">
                              ...
                              {payload.url && (
                                <div className="mt-4">
                                  <Button
                                    className="rounded-lg font-bold h-9 shadow-lg shadow-primary/10 hover:shadow-xl hover:translate-y-[-1px] transition-all"
                        ...
                        <div className="p-6 md:p-8 flex items-center justify-center md:border-l border-border bg-slate-50/50 dark:bg-slate-950/50">
                          <Button
                            variant={n.read ? "ghost" : "outline"}
                            size="icon"
                            onClick={() => n.read ? markUnreadMutation.mutate(n.id) : markReadMutation.mutate(n.id)}
                            className={cn(
                              "h-12 w-12 rounded-xl shadow-md transition-all duration-300",
