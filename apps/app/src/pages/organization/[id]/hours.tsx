...
export default function OrgVolunteerHoursPage() {
  ...
  const handleStatusUpdate = async (ids: number[], status: 'approved' | 'rejected', reason?: string) => {
    try {
      await api.post('/hours/bulk-status', {
        ids,
        status,
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between">
          ...
          {selectedIds.length > 0 && (
             <div className="flex gap-2">
                 <Button size="sm" onClick={() => handleStatusUpdate(selectedIds, 'approved')} className="bg-green-600 hover:bg-green-700">
                                ...
                                <TableCell className="text-right">
                                    {log.status === 'pending' && (
                                        <div className="flex justify-end gap-2">
                                            <Button size="icon" variant="ghost" className="h-8 w-8 text-green-600" onClick={() => handleStatusUpdate([log.id], 'approved')}>
                                                <CheckCircle2 className="h-4 w-4" />
                                            </Button>
                                            <Button size="icon" variant="ghost" className="h-8 w-8 text-red-600" onClick={() => setRejectParams({ ids: [log.id], open: true })}>
      ...
      <Dialog open={rejectParams.open} onOpenChange={(open) => !open && setRejectParams(prev => ({ ...prev, open: false }))}>
          <DialogContent>
               <DialogHeader>
               ...
               <DialogFooter>
                   <Button variant="outline" onClick={() => setRejectParams(prev => ({...prev, open: false}))}>Cancel</Button>
                   <Button variant="destructive" onClick={() => handleStatusUpdate(rejectParams.ids, 'rejected', rejectionReason)} disabled={!rejectionReason}>
                       Confirm Rejection
                   </Button>
               </DialogFooter>
          </DialogContent>
