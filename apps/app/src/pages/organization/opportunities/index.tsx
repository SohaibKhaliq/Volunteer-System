  ...
  const columns = [
    ...
    },
    {
        ...
        cell: ({ row }: any) => {
             ...
             return <span>{row.original.capacity > 0 ? `${row.original.application_count || 0} / ${row.original.capacity}` : 'Unlimited'}</span>
        }
    },
    {
        id: 'actions',
                    ...
                    <tbody className="[&_tr:last-child]:border-0">
                        {isLoading ? (
                            <tr><td colSpan={5} className="p-4 text-center">Loading...</td></tr>
                             ...
                             <tr><td colSpan={5} className="p-4 text-center text-muted-foreground">No opportunities found.</td></tr>
                        ) : (
                                 ...
                                 <tr key={opp.id} className="border-b transition-colors hover:bg-muted/50">
                                     ...
                                     <td className="p-4 text-right">
                                         <DropdownMenu>
                                           ...
                                           <DropdownMenuContent align="end">
                                             <DropdownMenuLabel>Actions</DropdownMenuLabel>
                                             <DropdownMenuItem onClick={() => navigate(`/volunteer/opportunities/${opp.id}`)}>View Public</DropdownMenuItem>
                                             <DropdownMenuItem onClick={() => navigate(`/organization/opportunities/${opp.id}/edit`)}>Edit Details</DropdownMenuItem>
                                             <DropdownMenuSeparator />
                                             <DropdownMenuItem className="text-red-500" onClick={() => deleteMutation.mutate(opp.id)}>Delete</DropdownMenuItem>
                                           </DropdownMenuContent>
                                         </DropdownMenu>
                                     </td>
                                 </tr>
