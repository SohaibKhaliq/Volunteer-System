    ...
    <div className="container mx-auto p-4 md:p-8 max-w-6xl space-y-10">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl">
        ...
        <div className="relative z-10 space-y-4 max-w-2xl">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight">{t('Recognized Impact')}</h1>
          <p className="text-lg md:text-xl text-indigo-100 font-medium leading-relaxed">
            {t(
              'Your certificates are a testament to your commitment and specialized skills. Keep track of your institutional recognition here.'
            )}
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground font-medium">
              {t('Track your progress through organization-certified training.')}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoadingTraining ? (
            ...
            trainingModules.map((item: any) => (
              <Card
                key={item.id}
                className="group border-border/50 rounded-[2rem] hover:shadow-2xl transition-all duration-500 overflow-hidden bg-white"
              >
                <CardContent className="p-6 space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h3 className="font-black text-lg group-hover:text-indigo-600 transition-colors">
                  ...
                  <div className="pt-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="w-full rounded-xl font-bold h-10 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-all border border-transparent group-hover:border-indigo-100"
                    >
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground font-medium">
            {t('Your verified achievements issued by registered organizations.')}
          </p>
        </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-dashed">
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="font-bold">
                          {cert.issuedAt || cert.created_at
                            ? format(new Date(cert.issuedAt || cert.created_at), 'MMM d, yyyy')
                            : t('N/A')}
                        </span>
                      </div>
                      {cert.module && (
