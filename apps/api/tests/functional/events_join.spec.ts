...
test.group('Event join flow', () => {
  test('authenticated user can join an event and create an assignment', async ({
    ...
    const user = await User.create({ email: `joiner-${Date.now()}@test`, password: 'pass', firstName: 'Test', lastName: 'User' })
    const organizer = await User.create({ email: `org-${Date.now()}@test`, password: 'pass', firstName: 'Test', lastName: 'User' })
