...
export default class AuditLog extends BaseModel {
   ...
   */
  public static async safeCreate(payload: Record<string, any>, trx?: any) {
    ...
    for (let attempts = 0; attempts < 5; attempts++) {
      try {
        ...
        const log = new this()
        if (trx) log.useTransaction(trx)
        log.fill(data as any)
    ...
    // @ts-ignore
    const finalLog = new this()
    if (trx) finalLog.useTransaction(trx)
    finalLog.fill(data as any)
    return await finalLog.save()
  }
