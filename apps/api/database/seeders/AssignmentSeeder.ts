...
export default class AssignmentSeeder extends BaseSeeder {
  public async run() {
    ...
    const sql = 'INSERT INTO assignments (task_id,user_id,status,quantity,notes,created_at,updated_at) VALUES ' + placeholders

    const bindings = rows.flatMap((row) => [row.task_id, row.user_id, row.status, row.quantity, row.notes, row.created_at, row.updated_at])
