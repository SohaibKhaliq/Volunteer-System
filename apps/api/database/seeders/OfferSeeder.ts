...
export default class OfferSeeder extends BaseSeeder {
  public async run() {
    ...
    const sql = 'INSERT INTO offers (user_id,title,description,category,status,location,contact_phone,availability,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE status=VALUES(status),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.user_id, row.title, row.description, row.category, row.status, row.location, row.contact_phone, row.availability, row.created_at, row.updated_at])
