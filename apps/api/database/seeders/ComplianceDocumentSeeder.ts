  ...
  public async run() {
    ...
    for (let i = 0; i < RECORD_COUNT; i++) {
      ...
      rows.push({
        ...
        is_active: 1,
        content: `This is the ${doc.title} document content for version ${version}.0`,
    ...
    const sql = 'INSERT INTO compliance_documents (organization_id,title,description,document_type,version,is_required,is_active,content,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE description=VALUES(description),is_active=VALUES(is_active),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.organization_id, row.title, row.description, row.document_type, row.version, row.is_required, row.is_active, row.content, row.created_at, row.updated_at])
