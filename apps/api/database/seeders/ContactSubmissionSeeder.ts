...
export default class ContactSubmissionSeeder extends BaseSeeder {
  public async run() {
    ...
    const sql = 'INSERT INTO contact_submissions (name,email,phone,subject,message,status,ip_address,user_agent,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE status=VALUES(status),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.name, row.email, row.phone, row.subject, row.message, row.status, row.ip_address, row.user_agent, row.created_at, row.updated_at])
