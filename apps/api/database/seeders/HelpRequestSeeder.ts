...
export default class HelpRequestSeeder extends BaseSeeder {
  public async run() {
    ...
    const sql = 'INSERT INTO help_requests (user_id,title,description,urgency,status,location,contact_phone,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE status=VALUES(status),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.user_id, row.title, row.description, row.urgency, row.status, row.location, row.contact_phone, row.created_at, row.updated_at])
