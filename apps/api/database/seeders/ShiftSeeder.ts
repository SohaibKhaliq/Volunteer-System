...
export default class ShiftSeeder extends BaseSeeder {
  public async run() {
    ...
    const sql = 'INSERT INTO shifts (title,description,event_id,organization_id,start_at,end_at,capacity,is_recurring,recurrence_rule,locked,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE description=VALUES(description),capacity=VALUES(capacity),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.title, row.description, row.event_id, row.organization_id, row.start_at, row.end_at, row.capacity, row.is_recurring, row.recurrence_rule, row.locked, row.created_at, row.updated_at])
