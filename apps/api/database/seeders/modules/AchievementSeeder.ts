...
export default class AchievementSeeder extends BaseSeeder {
  public async run() {
    ...
    const rows = achievements.slice(0, RECORD_COUNT).map((achievement) => ({
      ...
      // Generate key from name (slug)
      key: achievement.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''),
      description: achievement.description,
      category: achievement.category,
      points: achievement.points,
      icon: achievement.icon,
      requirement: achievement.requirement,
      // Infer rule type from requirement (e.g. 'hours_completed:10' -> 'hours')
      rule_type: achievement.requirement.split(':')[0].includes('hours') ? 'hours' : 
                 achievement.requirement.split(':')[0].includes('events') ? 'events' : 'custom',
      is_active: 1,
    ...
    const sql = 'INSERT INTO achievements (name,`key`,description,category,points,icon,requirement_json,rule_type,is_active,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE description=VALUES(description),points=VALUES(points),is_active=VALUES(is_active),rule_type=VALUES(rule_type),`key`=VALUES(`key`),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.name, row.key, row.description, row.category, row.points, row.icon, row.requirement, row.rule_type, row.is_active, row.created_at, row.updated_at])
