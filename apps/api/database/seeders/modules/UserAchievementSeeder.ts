  ...
  public async run() {
    ...
    while (count < RECORD_COUNT) {
      ...
      const metadata = JSON.stringify({
        awarded_via: grantedBy ? 'manual' : 'auto',
        milestone: isUnlocked ? `Completed ${progress}% progress` : null
      })
    ...
    const sql = 'INSERT INTO user_achievements (user_id,achievement_id,progress,unlocked_at,granted_by,grant_reason,metadata,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE progress=VALUES(progress),unlocked_at=VALUES(unlocked_at),metadata=VALUES(metadata),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.user_id, row.achievement_id, row.progress, row.unlocked_at, row.granted_by, row.grant_reason, row.metadata, row.created_at, row.updated_at])
