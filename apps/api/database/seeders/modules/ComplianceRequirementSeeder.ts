  ...
  public async run() {
    ...
    for (let i = 0; i < RECORD_COUNT; i++) {
      ...
      rows.push({
        ...
        enforcement_level: req.mandatory ? 'onboarding' : 'signup',
    ...
    const sql = 'INSERT INTO compliance_requirements (organization_id,name,description,doc_type,is_mandatory,enforcement_level,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE description=VALUES(description),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.organization_id, row.name, row.description, row.doc_type, row.is_mandatory, row.enforcement_level, row.created_at, row.updated_at])
