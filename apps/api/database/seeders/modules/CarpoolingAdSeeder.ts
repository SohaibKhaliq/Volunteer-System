...
export default class CarpoolingAdSeeder extends BaseSeeder {
  public async run() {
    ...
    for (let i = 0; i < RECORD_COUNT; i++) {
      ...
      rows.push({
        user_id: userId,
        departure_address: route.from,
        arrival_address: route.to,
        departure_date: departureDate.toISOString().slice(0, 19).replace('T', ' '),
        capacity: route.seats,
        price_per_seat: Math.floor(Math.random() * 20) + 5,
        status: status,
        description: 'Prefer non-smokers',
    ...
    const sql = 'INSERT INTO carpooling_ads (user_id,departure_address,arrival_address,departure_date,capacity,price_per_seat,status,description,created_at,updated_at) VALUES ' + placeholders +
      ' ON DUPLICATE KEY UPDATE status=VALUES(status),updated_at=VALUES(updated_at)'

    const bindings = rows.flatMap((row) => [row.user_id, row.departure_address, row.arrival_address, row.departure_date, row.capacity, row.price_per_seat, row.status, row.description, row.created_at, row.updated_at])
