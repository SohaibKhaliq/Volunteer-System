...
export default class ResourceSeeder extends BaseSeeder {
  public async run() {
    ...
    const resources = [
      ...
      {
        name: 'PA System with Microphones',
        category: 'Technology',
        qty: 2,
    ...
    const rows = resources.slice(0, RECORD_COUNT).map((res, index) => {
      ...
      const createdById =
        userIds.length > 0 ? userIds[Math.floor(Math.random() * userIds.length)] : null
      const attributes = JSON.stringify({
        brand:
          res.category === 'Transport' ? 'Toyota' : res.category === 'Technology' ? 'Dell' : null,
        condition: ['excellent', 'good', 'fair'][Math.floor(Math.random() * 3)],
        purchase_date: new Date(Date.now() - Math.random() * 365 * 3 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split('T')[0]
      })
    ...
    const sql =
      'INSERT INTO resources (organization_id,name,category,description,quantity_total,quantity_available,location,serial_number,status,maintenance_due,created_by_id,attributes,created_at,updated_at) VALUES ' +
      placeholders +
      ' ON DUPLICATE KEY UPDATE description=VALUES(description),quantity_total=VALUES(quantity_total),quantity_available=VALUES(quantity_available),status=VALUES(status),attributes=VALUES(attributes),updated_at=VALUES(updated_at)'
