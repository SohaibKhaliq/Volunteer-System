...
export default class ResourceSeeder extends BaseSeeder {
  public async run() {
    ...
    const rows = resources.slice(0, RECORD_COUNT).map((res, index) => {
      ...
      const createdById = userIds.length > 0 ? userIds[Math.floor(Math.random() * userIds.length)] : null
      const attributes = JSON.stringify({
        brand: res.category === 'Transport' ? 'Toyota' : res.category === 'Technology' ? 'Dell' : null,
        condition: ['excellent', 'good', 'fair'][Math.floor(Math.random() * 3)],
        purchase_date: new Date(Date.now() - Math.random() * 365 * 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
      })
