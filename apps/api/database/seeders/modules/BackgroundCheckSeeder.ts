...
export default class BackgroundCheckSeeder extends BaseSeeder {
  public async run() {
    ...
    const sql = 'INSERT INTO background_checks (user_id,status,issued_at,expires_at,reference_number,notes,created_at,updated_at) VALUES ' + placeholders +
    ...
    const bindings = rows.flatMap((row) => [row.user_id, row.status, row.issued_at, row.expires_at, row.reference_number, row.notes, row.created_at, row.updated_at])
